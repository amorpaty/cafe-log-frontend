import {
  Lock,
  EyeOff,
  ArrowLeft,
  Coffee,
  User,
  Eye,
} from "lucide-react";
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import { postCallApi } from "../../common/utils/apiUtils/apiUtils";
import { setStorageUserId } from "../../common/utils/utils";

/**
 * @author 남은주
 * @since 26.07.05
 * @description 로그인 페이지 입니다.
 * @returns 
 */
function LoginPage() {

    const navigate = useNavigate();

    const [nickname, setNickname] = useState("");
    const [pin, setPin] = useState("");
    const [showPin, setShowPin] = useState(false);

    const goAccount = () => {
        navigate("/account")
    }

    const handlePinChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value.replace(/[^0-9]/g, "");
        setPin(value);
    };

    const handleNicNameChange = (e : React.ChangeEvent<HTMLInputElement>) => {
        const value = e.target.value.replace(/[^a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]/g, "");
        setNickname(value);
    }

    const handleLogin = async () => {
        // 회원가입 API 호출 예정
        if (nickname.length < 2) {
            alert("닉네임은 2자 이상 입력해주세요.");
            return;
        }

        if(/[^a-zA-Z0-9ㄱ-ㅎㅏ-ㅣ가-힣]/g.test(nickname)){
            alert("닉네임은 한글, 영문, 숫자만 입력 가능합니다.");
            return;
        }

        if (!/^\d{8}$/.test(pin)) {
            alert("PIN은 숫자 8자리입니다.");
            return;
        }

        const res = await postCallApi("/users/login", {nickname, pin});

        if(res.data.code !== 0){
            alert(res.data.msg);
            return;
        }else{
            setStorageUserId(res.data.user_id);
            navigate("/main");
        }
    }

    return (
        <div className="min-h-screen bg-[#FAF8F3] flex items-center justify-center px-6">
            <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-sm">
                <button className="mb-4">
                    <ArrowLeft className="text-gray-500 cursor-pointer" onClick={() => navigate("/")}/>
                </button>
                <div className="mb-10 text-center">
                    <div className="flex mb-4 text-5xl items-center justify-center"><Coffee size={70} className="text-[#D4A017]"/></div>

                    <div className="mb-4 mt-4">
                        <span className="text-4xl font-bold text-[#D4A017]">카공로그</span>
                    </div>
                    <div className="mt-4">
                        <p className="mt-4 text-sm font-bold leading-6 text-gray-500">
                            로그인하고 나의
                            <br />
                            공부 기록을 확인해보세요.
                        </p>
                    </div>
                </div>

                <div className="space-y-4">
                    <div className="flex items-center rounded-xl border border-gray-200 bg-white px-4 py-4">
                        <User size={18} className="mr-3 text-gray-400" />
                        <input 
                            type="text"
                            maxLength={10}
                            value={nickname}
                            onChange={(e) => handleNicNameChange(e)}
                            placeholder="닉네임을 입력해주세요"
                            onKeyDown={(e) => {
                                if(e.key !== "Enter")
                                    return;

                                handleLogin();
                            }}  
                            className="w-full outline-none placeholder:text-gray-400"
                        />
                    </div>

                    <div className="flex items-center rounded-xl border border-gray-200 bg-white px-4 py-4">
                        <Lock size={18} className="mr-3 text-gray-400" />
                        <input
                            type={showPin ? "text" : "password"}
                            value={pin}
                            maxLength={8}
                            inputMode="numeric"
                            placeholder="PIN 8자리 입력해주세요"
                            onChange={handlePinChange}
                            onKeyDown={(e) => {
                                if(e.key !== "Enter")
                                    return;

                                handleLogin();
                            }}  
                            className="w-full outline-none placeholder:text-gray-400"
                        />
                        {
                            showPin ? (
                                <Eye
                                size={18}
                                className="cursor-pointer text-gray-400"
                                onClick={() => setShowPin(false)}
                                />
                            ) : (
                                <EyeOff
                                size={18}
                                className="cursor-pointer text-gray-400"
                                onClick={() => setShowPin(true)}
                                />
                            )
                        }
                    </div>

                    <div className="flex items-center justify-between text-sm">
                        <label className="flex items-center gap-2 text-gray-600">
                        <input type="checkbox" className="h-4 w-4 accent-[#D4A017]" />
                        로그인 상태 유지
                        </label>

                        <button className="font-medium text-[#D4A017]">
                        비밀번호 찾기
                        </button>
                    </div>

                     {/* hover:bg-[#BF8F0A] cursor-pointer */}
                    <button className="w-full rounded-xl bg-[#D4A017] py-4 font-semibold text-white transition hover:bg-[#BF8F0A] cursor-pointer"
                        onClick={handleLogin}
                    >
                        로그인
                    </button>

                </div>
                <div className="mt-8 text-center text-sm text-gray-500">
                    아직 계정이 없으신가요?
                    <button className="ml-2 font-semibold text-[#D4A017] hover:text-[#BF8F0A] cursor-pointer" 
                        onClick={goAccount}
                    >
                        회원가입
                    </button>
                </div>
            </div>
        </div>
    );
}

export default LoginPage;
